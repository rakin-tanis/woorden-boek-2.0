...
import { JokerButtonVariantsDetail, JokerButtonVariantType } from './jokerVariants';
import React from 'react';
import { LucideIcon } from 'lucide-react';

export const JokerWinAnimation: React.FC<{
  jokers: { name: string, icon: LucideIcon, count: number, variant: JokerButtonVariantType }[],
}> = ({ jokers }) => {
    ...
    <div className="absolute inset-0 bottom-16 top-16 z-50 flex flex-row items-center justify-center gap-4 flex-wrap">
      {/* <div className="p-20 rounded-full relative flex flex-row flex-wrap gap-4"> */}
        ...
        {jokers.map((joker, index) => {
          const colorClass = 'text-' + (JokerButtonVariantsDetail[joker.variant]?.color || 'text-white');
          return (
            <motion.div
              key={joker.name}
              ...
              {React.createElement(joker.icon, {
                className: `${colorClass} w-12 h-12`,
                style: {
                  textShadow: 'rgba(255,255,0,0.9) 0px 0px 12px',
